...
// UPDATED: Header keys sesuai dengan kode baru
const HEADER_KEYS = {
  META_HASH: "09d",        // Changed from "x-xcsa3d" to "09d"
  CLIENT_TIME: "xcsad23",  // Changed from "x-dbsv" to "xcsad23"
  TRACE_ID: "aaa-3bsv"     // Changed from "x-dsa" to "aaa-3bsv"
};
...
// UPDATED: Utility Functions with new auth system
function buildSecret() {
  const parts = ["bbsds!eda", "2", "3ed2@#@!@#Ffdf#@!", "4"];
  const pattern = [2, 1, 0, 2, 1, 2]; // Pattern tetap sama dari kode asli
...
// UPDATED: createSignature - tetap sync untuk Node.js compatibility
function createSignature(secret, message) {
  return crypto.createHmac("sha256", secret).update(message).digest("hex");
}

// UPDATED: createAuthHeaders - tetap sync tapi siap untuk async jika diperlukan
...
// UPDATED: Base headers helper function
function createBaseHeaders(cookieString, authToken = null) {
  const headers = {
  ...
  // Tambahkan authorization header jika ada auth token (sesuai kode baru)
  if (authToken) {
...
// UPDATED: CAPTCHA Handler Class dengan header baru
class CaptchaHandler {
  ...
  // UPDATED: verifyCaptcha dengan header baru
  async verifyCaptcha(token, cookieString, authToken = null) {
    const baseHeaders = createBaseHeaders(cookieString, authToken);
    
    // UPDATED: Tambah header khusus untuk CAPTCHA verification
    const headers = {
      ...baseHeaders,
      "x-trace-id": crypto.randomUUID(), // Tambahan untuk CAPTCHA
    };
  ...
  async handleCaptchaError(error, cookieString, authToken = null) {
    ...
    if (errorMessage.includes("captcha verification required") || 
      ...
      try {
        ...
        await this.verifyCaptcha(captchaToken, cookieString, authToken);
...
// Cookie Manager Class - tidak berubah
class CookieManager {
  constructor() {
    this.cookies = [];
  }
...
// UPDATED: Game State Manager Class - Enhanced with Smart Booster Strategy and New Auth Headers
class GameStateManager {
  constructor(cookieString = null, proxyUrl = null, authToken = null) {
    this.cookieString = cookieString;
    this.proxyUrl = proxyUrl;
    this.authToken = authToken; // UPDATED: Tambah support untuk auth token
    this.gameState = null;
    this.userInfo = null;
  ...
  // UPDATED: Tambah method untuk set auth token
  ...
  // UPDATED: getState method dengan header baru
  async getState() {
    if (!this.cookieString) {
      throw new Error("Cookie belum di-set. Gunakan setCookie() atau loadCookieFromFile()");
    }

    // UPDATED: Gunakan createBaseHeaders helper
    const headers = createBaseHeaders(this.cookieString, this.authToken);
  ...
  // Plant Strategy Methods - tidak berubah
...
// Prestige Methods - UPDATED dengan header baru
...
// UPDATED: performPrestige dengan header baru
GameStateManager.prototype.performPrestige = async function() {
  ...
  try {
    ...
    // UPDATED: Gunakan createBaseHeaders dan createAuthHeaders baru
    const baseHeaders = createBaseHeaders(this.cookieString, this.authToken);
    ...
    if (error.message.includes('412') || error.message.includes('captcha')) {
      try {
        ...
        const handled = await captchaHandler.handleCaptchaError(error, this.cookieString, this.authToken);
...
// Farmhouse Methods - UPDATED dengan header baru
...
// UPDATED: claimFarmhouse dengan header baru
...
// UPDATED: Buy Item Method dengan header baru
...
// UPDATED: Buy Plot Method dengan header baru
...
// UPDATED: Plant Seed Method dengan header baru
...
// UPDATED: Harvest Methods dengan header baru
...
// Auto Plant Seeds - tidak berubah
...
// UPDATED: Apply Booster Method dengan header baru
...
class AccountWorker {
  ...
  // FIXED: processAccount dengan TRUE Plants-First Flow dan header baru
...
// Main Functions dengan Smart Booster Strategy dan UPDATED headers
async function runSingleAccount(accountIndex = 0) {
  ...
  try {
    ...
    console.log("\n=== MENJALANKAN SMART-BOOSTER STRATEGY (UPDATED HEADERS) ===");
...
// Test Function untuk Debug Smart Booster dengan UPDATED headers
async function testSmartBooster(accountIndex = 0) {
  ...
  try {
    ...
    console.log("=== SMART BOOSTER TEST (UPDATED HEADERS) ===");
    console.log("Before:");
    ...
    console.log("\n--- Executing Smart Booster with UPDATED Headers ---");
...
// UPDATED: Test function untuk header baru
async function testNewHeaders(accountIndex = 0) {
  const stateManager = new GameStateManager();
  
  try {
    await stateManager.loadCookieFromFile(accountIndex);
    console.log("=== TESTING NEW HEADERS ===");
    
    // Test basic state fetch
    console.log("Testing getState with new headers...");
    const state = await stateManager.getState();
    
    if (state.gameState && state.userInfo) {
      console.log("✓ New headers working - state fetched successfully");
      console.log(`AP: ${state.gameState.ap}, Prestige: ${state.userInfo.prestigeLevel}`);
    } else {
      console.log("✗ New headers may have issues - no state data");
    }
    
    // Test auth headers generation
    console.log("\nTesting auth headers generation...");
    const testInput = { test: "data" };
    const authHeaders = createAuthHeaders(testInput);
    
    console.log("Generated headers:");
    console.log(`- ${HEADER_KEYS.META_HASH}: ${authHeaders[HEADER_KEYS.META_HASH]}`);
    console.log(`- ${HEADER_KEYS.CLIENT_TIME}: ${authHeaders[HEADER_KEYS.CLIENT_TIME]}`);
    console.log(`- ${HEADER_KEYS.TRACE_ID}: ${authHeaders[HEADER_KEYS.TRACE_ID]}`);
    
    console.log("✓ Header generation test completed");
...
// Export - UPDATED dengan function baru
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    CONFIG,
    HEADER_KEYS, // UPDATED: Export header keys baru
    generateNonce,
    createSignature,
    createAuthHeaders,
    createBaseHeaders, // UPDATED: Export helper function
    makeHttpRequest,
    ProxyManager,
    CaptchaHandler,
    CookieManager,
    ...
    startParallelBot,
    testSmartBooster,
    testNewHeaders // UPDATED: Export test function baru
  };
}

// Auto start dengan informasi Smart Booster Strategy dan UPDATED Headers
if (require.main === module) {
  console.log("=".repeat(80));
  console.log("APPLEVILLE PARALLEL BOT v3.3 - SMART-BOOSTER + UPDATED HEADERS");
  console.log("=".repeat(80));
  console.log("");
  
  console.log("🔐 UPDATED AUTHENTICATION HEADERS:");
  console.log(`  • META_HASH: "${HEADER_KEYS.META_HASH}" (was: "x-xcsa3d")`);
  console.log(`  • CLIENT_TIME: "${HEADER_KEYS.CLIENT_TIME}" (was: "x-dbsv")`);
  console.log(`  • TRACE_ID: "${HEADER_KEYS.TRACE_ID}" (was: "x-dsa")`);
  console.log("  • Added: Auth token support for authorization header");
  console.log("");
  ...
  console.log("  ✓ NEW: Updated authentication headers");
  console.log("  ✓ NEW: Auth token support");
  console.log("");
  ...
  console.log("  • Updated headers sesuai dengan kode terbaru");
  console.log("");
  
  console.log("🧪 TEST COMMANDS:");
  console.log("  • testNewHeaders(0) - Test header baru untuk akun 1");
  ...
  console.log("Starting SMART-BOOSTER bot with UPDATED HEADERS in 3 seconds...");
  console.log("=".repeat(80));
